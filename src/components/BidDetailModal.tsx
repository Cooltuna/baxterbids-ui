...
`;
  ...
  const fetchPricingIntel = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/bids/${encodeURIComponent(bid.id)}/enrich`, {
        method: 'POST',
      ...
      setEnrichError('Failed to connect to API');
    } finally {
              ...
              <div>
                {bid.enrichment?.highergov ? (
                  <div className="space-y-6">
                    ...
                    {/* Extract BOM - quick action from Intel tab */}
                    <div className="pt-4 border-t border-[var(--border)]">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-medium text-[var(--foreground)]">Next Step</p>
                          <p className="text-sm text-[var(--muted)]">Extract the Bill of Materials from the bid summary</p>
                        </div>
                        <button
                          onClick={() => handleAnalyze(false)}
                          disabled={isAnalyzing || apiAvailable === false}
                          className="inline-flex items-center gap-2 px-5 py-2.5 bg-[var(--accent)] text-white rounded-lg font-medium hover:bg-[var(--accent)]/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          {isAnalyzing ? (
                            <>
                              <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                    ...
                    {previewDoc.type?.toLowerCase() === 'pdf' ? (
                      ...
                      <div className="p-8 text-center bg-[var(--background)]">
                        ...
                        <p className="text-[var(--muted)] mb-4">
                          Excel/Word files can&apos;t be previewed inline
                        </p>
                        <div className="flex items-center justify-center gap-3">
                          <a
                            href={`https://view.officeapps.live.com/op/view.aspx?src=${encodeURIComponent(previewDoc.url)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="px-4 py-2 text-sm font-medium rounded-lg bg-[var(--accent)] text-white hover:bg-[var(--accent)]/90 transition-colors"
                          >
                  ...
                  <div className="mt-6">
                    <CustomVendorInput 
                      onDraftRFQ={(name, email) => {
                        handleDraftEnrichmentRFQ({ name, cage: '' });
                      }}
                    />
                  </div>
                )}
